...
import {img_300, img_not_available} from '../../Configure';
const CardMoviesComponents = ({data,mediaType})=>{
    //console.log(data)

    const title = data.original_title || data.name;//title
    const id = data.id;
    const ImageURL =  data.poster_path ? img_300+ data.poster_path : img_not_available;//image 
    const media_type = data.media_type ? data.media_type : data.type ? data.type : mediaType;//media type
    const release_date =  data.release_date || data.first_air_date;//released year
    const vote_average = parseInt(data.vote_average);//Converting from decimal number to integer
    const original_language = data.original_language || ''//language
    return (
        <>
